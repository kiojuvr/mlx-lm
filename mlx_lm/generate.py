...
DEFAULT_MTP_ADAPTIVE_FALLBACK_MIN_DRAFTED_TOKENS = 8
DEFAULT_MTP_ADAPTIVE_FALLBACK_MIN_ACCEPTANCE_RATE = 0.50
...
class GenerationResponse:
    ...
    Args:
        ...
        logprobs (Optional[mx.array]): A vector of log probabilities. This is
            ``None`` when an optimized generation path was explicitly told
            that log probabilities are not required.
...
def mtp_speculative_generate_step(
    ...
    *,
    ...
    skip_target_logprobs = (
        bool(mtp_sampler_is_greedy)
        and not use_logits_processors
        and not mtp_return_logprobs
    )
    ...
    with mx.stream(generation_stream):
        ...
        mx.async_eval(
            *(
                (current, previous_hidden)
                if logprobs is None
                else (current, logprobs, previous_hidden)
            )
        )
    ...
    while _can_emit():
            ...
            with mx.stream(generation_stream):
                ...
                mx.async_eval(
                    *(
                        ...
                        if next_logprobs is None
                        else (next_token, next_logprobs, next_hidden)
                    )
                )
        ...
        if mtp_sampler_is_greedy and not use_logits_processors:
            ...
            if skip_target_logprobs:
                target_logprobs = [None] * needed_tokens
                stats["target_logsumexp_skipped"] += needed_tokens
            else:
