...
template <
    ...
    const int BK = 32,
    const int BN = 32>
[[kernel]] void affine_qmm_t_head_broadcast(
    const device uint32_t* w [[buffer(0)]],
    const device T* scales [[buffer(1)]],
    const device T* biases [[buffer(2)]],
    const device T* x [[buffer(3)]],
...
template <
    ...
    const int BK = 32,
    const int BN = 32>
[[kernel]] void affine_qmm_t_head_broadcast_heads(
    const device uint32_t* w [[buffer(0)]],
    const device T* scales [[buffer(1)]],
    const device T* biases [[buffer(2)]],
    const device T* x [[buffer(3)]],
...
template <
    ...
    const int BK = 32,
    const int BN = 32>
[[kernel]] void affine_qmm_t_head_broadcast_split(
    const device uint32_t* w [[buffer(0)]],
    const device T* scales [[buffer(1)]],
    const device T* biases [[buffer(2)]],
    const device T* x [[buffer(3)]],
    device T* y_nope [[buffer(4)]],
    device T* y_pe [[buffer(5)]],
  ...
  const int h = tid.z % uint(H);
  const int b = tid.z / uint(H);
  const bool is_pe = y_col >= nope_N;
  const int out_N = is_pe ? pe_N : nope_N;
  const int out_col = is_pe ? y_col - nope_N : y_col;
  ...
  const device T* biases_h = biases + (size_t(h) * N + y_col) * K_g;
  device T* y_head =
      (is_pe ? y_pe : y_nope) + (size_t(b) * H + h) * M * out_N;

  x_batch += y_row * static_cast<int64_t>(K);
  y_head += y_row * static_cast<int64_t>(out_N) + out_col;

  const short num_els = min(BM, M - y_row);
  const short num_outs = min(BN, out_N - out_col);
  ...
  threadgroup_barrier(mem_flags::mem_threadgroup);
  if (num_els < BM || num_outs < BN) {
    mma_op.store_result_safe(y_head, out_N, short2(num_outs, num_els));
  } else {
    mma_op.store_result(y_head, out_N);
  }
}
...
template <
    ...
    const int BK = 32,
    const int BN = 32>
[[kernel]] void affine_qmm_t_head_broadcast_heads_scaled(
    const device uint32_t* w [[buffer(0)]],
    const device T* scales [[buffer(1)]],
    const device T* biases [[buffer(2)]],
    const device T* x [[buffer(3)]],
...
template <typename T, const int D = 2048, const int THREADS = 256>
[[kernel]] void glm_rms_norm_inv_scale_2048(
    const device T* x [[buffer(0)]],
    device T* y [[buffer(1)]],
  ...
  if (simd_gid == 0) {
    total = simd_sum(total);
    if (simd_lid == 0) {
      y[row] = T(rsqrt(total / float(D) + eps));
    }
  }
}
