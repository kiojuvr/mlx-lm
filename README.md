...
keeps it disabled while practical output stability is being compared against
target-only decode. The historical decode numbers below remain
...
as an experimental single-request OpenCode path. On the tested 8K-context,
...
After eight drafted tokens, the default adaptive guard falls back to regular
one-token target decode when observed MTP acceptance remains below `0.50`. This
