...
class TestServerCLI(unittest.TestCase):
    ...
    def test_single_request_uses_mtp_ram_cache_namespace(self):
        generator = ResponseGenerator.__new__(ResponseGenerator)
        cli_args = types.SimpleNamespace(
            prefill_step_size=2048,
            prefill_max_qk_tokens=67_108_864,
        ...
        model_key = ("model", None, None)
        mtp_model_key = (model_key, "mtp-speculative")
        generator.model_provider = types.SimpleNamespace(
            model=object(),
            tokenizer=tokenizer,
            draft_model=None,
        ...
        class FakePromptCache:
            ...
            def fetch_nearest_cache(self, model_key, tokens):
                self.fetched = (model_key, tokens)
                return mtp_cache, rest
        ...
        while not rqueue.empty():
            queued.append(rqueue.get())
        self.assertFalse(any(isinstance(item, Exception) for item in queued))
        self.assertEqual(generator.prompt_cache.fetched, (mtp_model_key, prompt))
        self.assertEqual(captured["prompt"], rest)
        ...
        self.assertEqual(
            ...
            "mtp-speculative-server-cache",
        )
        self.assertEqual(
            generator.prompt_cache.inserted,
            (mtp_model_key, prompt, mtp_cache),
        )
    ...
    def test_single_request_uses_mtp_rendered_exact_before_ram_cache(self):
        generator = ResponseGenerator.__new__(ResponseGenerator)
        cli_args = types.SimpleNamespace(
            prefill_step_size=2048,
            prefill_max_qk_tokens=67_108_864,
        ...
        disk_cache = [FakeCache(offset=3)]
        ram_cache = [FakeCache(offset=2)]
        rendered_checkpoint = RenderedPromptCheckpoint(
            ...
            kind="exact",
            rendered_prefix_bytes=3,
            checkpoint_path="/tmp/mtp-exact-3.safetensors",
        )
        ...
        self.assertEqual(generator.prompt_cache.fetched, (mtp_model_key, prompt))
        self.assertEqual(captured["prompt"], prompt[3:])
        self.assertIs(captured["prompt_cache"], disk_cache)
        self.assertTrue(captured["mtp_speculative"])
        self.assertEqual(captured["prompt_checkpoint_full_prompt"], prompt)
        self.assertEqual(captured["prompt_checkpoint_initial_cached_tokens"], 3)
        self.assertEqual(
            captured["prompt_checkpoint_initial_cache_source"],
            "disk-rendered-mtp-exact",
        )
        self.assertEqual(
            generator.prompt_cache.inserted,
            (mtp_model_key, prompt, disk_cache),
        )
